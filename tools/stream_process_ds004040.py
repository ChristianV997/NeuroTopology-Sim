#!/usr/bin/env python3
"""Stream-process ds004040 (Trance channeling EEG study) one subject at a time.

Dedicated runner (like ds004541/ds005555/ds001787): ds004040's state comes
from real rest/trance block-boundary markers in each recording's
events.tsv `value` column (`rest1_start`/`rest1_end`/`trance1_start`/... --
`trial_type` is always the literal "STATUS", not usable directly), windowed
by `sciencer_d/btc_icft/level_m/ds004040_windows_real.py` -- not from a BIDS
task entity (every recording is `task-trance`), so it can't use the generic
registry-driven windower.

13 subjects, 2 sessions each (`ses-01`, `ses-02`), 3 rest + 3 trance blocks
per session. 64-channel Biosemi EEG, standard 10-20 channel names.
"""
...
def process_subject(
    ...
    from sciencer_d.btc_icft.level_m.ds004040_windows_real import build_and_extract_real_windows
...
def main() -> int:
    ...
    p.add_argument("--out", default="outputs/btc_icft/ds004040/stream")
    p.add_argument("--work-root", default="data/ds004040")
