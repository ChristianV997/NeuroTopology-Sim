...
| ds004040 (trance channeling EEG) | channel-mean topology (q_net/q_abs/f_dress) | ✓ | ✓ | **n=13, null** (rest vs trance, |d|<0.011) | Onboarded; see `REPORT_ds004040_trance_channeling_topology.md` |
| ds006644 (DMT+harmine, meditation retreat fMRI) | persistent homology + graph metrics (`dual_engine/fmri_tda_pipeline.py`) | ✓ (synthetic) | n/a | **n=40, marginal** (global_efficiency p=0.0496, does not survive Bonferroni×4) | First real-data run of this pipeline; see `REPORT_ds006644_dmt_meditation_fmri_topology.md` |
| ds005917 (NIMH ketamine, MDD group fMRI) | persistent homology + graph metrics (`dual_engine/fmri_tda_pipeline.py`) | ✓ (synthetic) | n/a | **n=24, null** (drug vs placebo, paired, |d_z|≤0.29) | No spatial-normalization derivatives available; per-subject KMeans fallback used (disclosed); see `REPORT_ds005917_ketamine_fmri_topology.md` |
| DANDI 000458 (mouse EEG, isoflurane) | channel-mean topology (q_net/q_abs/f_dress) | ✓ | ✓ | **n=23, suggestive** (awake>anesthetized all metrics; MixedLM q_net p=0.002, but subject-blocked ns) | First DANDI/NWB dataset (lazy HTTP NWB streaming); new cross-species anesthesia axis; see `REPORT_dandi000458_isoflurane_topology.md` |
