...
def _mock_m_rows() -> list[dict]:
    return [
        {"row_id":"sub-01_ses-01_run-01_awake_win-0","subject_id":"sub-01","session_id":"ses-01","run_id":"01","window_id":"win-0","task_label":"awake","source_file":"mock/a.edf","window_start_s":"0","window_end_s":"10","artifact_score":"0.1"},
        {"row_id":"sub-01_ses-01_run-01_sedated_win-1","subject_id":"sub-01","session_id":"ses-01","run_id":"01","window_id":"win-1","task_label":"sedated","source_file":"mock/a.edf","window_start_s":"10","window_end_s":"20","artifact_score":"0.6"},
        {"row_id":"sub-02_ses-01_run-01_awake_win-0","subject_id":"sub-02","session_id":"ses-01","run_id":"01","window_id":"win-0","task_label":"awake","source_file":"mock/b.edf","window_start_s":"0","window_end_s":"10","artifact_score":"0.2"},
    ]


def main() -> int:
    p = argparse.ArgumentParser()
    p.add_argument("--m-windows", default="outputs/btc_icft/ds005620/m_real")
    ...
    res = topo.LevelTRealTopologyResult(
        dataset_id="ds005620", n_rows=len(rows), n_subjects=len({r.subject_id for r in rows}), n_windows=len(rows),
        topology_quality_report=q, null_placeholder_report=n, artifact_alignment_report=ar, omega_event=omega,
        safe_claim="Local DS005620-style EEG windows were mapped into operational Level T topology telemetry candidates for future M+T residual testing.",
