"""ds001787 (expert vs novice meditation, with continuous depth-of-meditation probes)
Level M window scaffold.

Two distinct window row shapes share one dataclass (fields left None when not
applicable for a given mode -- keeps a single CSV schema Level T's loader can
always consume via REQUIRED_M_COLUMNS, per the existing per-dataset contract):

- "fixed" mode: windows evenly spaced across the whole recording, independent
  of probe timing/alignment. Used for Analysis A (between-subject trait:
  expert vs novice), because it does not depend on the cross-clock alignment
  that fails for 5/41 sessions (see ds001787_behavioral.py) and keeps window
  count/length uniform across ALL subjects -- avoiding a window-length
  confound between subjects that DO and don't have usable probe alignment.
- "probe_locked" mode: one window per successfully-aligned probe, spanning
  from the previous aligned probe (or recording start) to this probe's
  events.tsv-relative stimulus onset, labeled with that probe's
  depth-of-meditation rating (0-3). Used for Analysis B (within-recording
  depth correlation). Only emitted for subject/sessions whose alignment
  passes ALIGNMENT_MATCH_RATE_GATE.
"""
...
class LevelMWindowRow:
    ...
    window_id: str
    task_label: str | None
    state_label: str | None  # "expert" | "novice" (trait group, from participants.tsv)
    behavior_label: str | None  # unused for this dataset; kept for schema parity
    report_label: str | None  # unused for this dataset; kept for schema parity
    y: int | None
    ...
    window_end_s: float
    warnings: list[str]
    window_mode: str = "fixed"  # "fixed" | "probe_locked"
    depth_of_meditation: int | None = None  # 0-3, probe_locked only
    depth_of_mind_wandering: int | None = None
    tiredness: int | None = None
...
def evaluate_level_m_windows(rows: list[LevelMWindowRow], task: str = "expert_vs_novice") -> LevelMRealWindowResult:
    warnings: list[str] = []
    selected: list[LevelMWindowRow] = []
    for row in rows:
        y = None
        if task == "expert_vs_novice":
            if row.state_label == "expert":
                y = 0
            elif row.state_label == "novice":
                y = 1
        else:
    ...
    safe_claim = "Local DS001787-style files were mapped into operational Level M window-feature candidates for future residual testing."
    forbidden_claims = [
    ...
    omega_event = {"dataset_id": "ds001787", "task": task, "status": "operational_level_m", "safe_claim": safe_claim}
    return LevelMRealWindowResult(
        dataset_id="ds001787", task=task, n_rows=len(selected), n_subjects=len({r.subject_id for r in selected}), n_windows=len(selected),
...
def write_level_m_window_outputs(result: LevelMRealWindowResult, out_dir: str) -> dict[str, str]:
    ...
    report_text = "\n".join([
        "# DS001787 Real/Local Level M Window Extraction",
        "## Dataset/task",
        f"- dataset_id: {result.dataset_id}",
        f"- task: {result.task}",
        "## Input",
        "- Source: real BIDS EEG signal (expert vs novice meditation, with probe-locked depth ratings where alignment permits).",
        "## Window rows",
