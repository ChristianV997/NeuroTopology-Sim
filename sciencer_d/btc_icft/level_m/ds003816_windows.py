"""ds003816 (loving-kindness meditation, LKM, long-term vs short-term
practitioners) Level M window scaffold.

Mirrors `ds003969_windows.py`'s structure (LevelMWindowRow, evaluate/report
helpers). Confirmed real BIDS task labels (via direct S3 listing of
sub-01lt/ses-01..02/eeg/, not assumed): `LKMSelf`, `LKMOther` (loving-
kindness meditation directed at self/other), `PreResting`, `PostResting`
(resting baselines), `VisualizeSelf`, `VisualizeOther` (a visualization
control condition -- neither meditation nor pure rest, deliberately left
unmapped/excluded from the 2-class contrast below rather than guessed into
either bucket). No `acq` BIDS entity; subjects have multiple `ses-NN`
sessions (long-term practitioners recorded up to 10 times).

Subject IDs carry a `lt`/`st` suffix (long-term/short-term practitioner)
per the dataset's own README, not used for the state contrast here.
"""
...
# Confirmed via direct S3 listing of ds003816 (sub-01lt/ses-01, ses-02 eeg/ dirs):
# task-LKMSelf, task-LKMOther, task-PreResting, task-PostResting,
# task-VisualizeSelf, task-VisualizeOther. Not assumed. Shown here in their
# real BIDS casing for documentation; the actual lookup in
# ds003816_windows_real.py lowercases before matching.
_TASK_TO_STATE = {
    "LKMSelf": "meditation",
    "LKMOther": "meditation",
    "PreResting": "resting",
    "PostResting": "resting",
    # VisualizeSelf/VisualizeOther intentionally unmapped: a visualization
    # control condition, neither loving-kindness meditation nor a passive
    # resting baseline -- excluded from the 2-class contrast rather than
    # guessed into either bucket (no_label_inference guardrail).
}
...
def evaluate_level_m_windows(rows: list[LevelMWindowRow], task: str = "lkm_vs_resting") -> LevelMRealWindowResult:
    ...
    for row in rows:
        y = None
        if task == "lkm_vs_resting":
            if row.state_label == "meditation":
                y = 0
            elif row.state_label == "resting":
                y = 1
        else:
    ...
    safe_claim = "Local DS003816-style files were mapped into operational Level M window-feature candidates for future residual testing."
    forbidden_claims = [
    ...
    omega_event = {"dataset_id": "ds003816", "task": task, "status": "operational_level_m", "safe_claim": safe_claim}
    return LevelMRealWindowResult(
        dataset_id="ds003816", task=task, n_rows=len(selected), n_subjects=len({r.subject_id for r in selected}), n_windows=len(selected),
...
def write_level_m_window_outputs(result: LevelMRealWindowResult, out_dir: str) -> dict[str, str]:
    ...
    report_text = "\n".join([
        "# DS003816 Real/Local Level M Window Extraction",
        "## Dataset/task",
        f"- dataset_id: {result.dataset_id}",
        f"- task: {result.task}",
        "## Input",
        "- Source: real BIDS EEG signal (loving-kindness meditation vs resting blocks).",
        "## Window rows",
