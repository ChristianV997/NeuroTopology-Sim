"""Real signal-derived Level T topology for ds001787 (expert vs novice meditation).

Direct port of the ds005620/ds003969 pattern. `compute_real_topology_for_window`
reads the actual per-channel signal for a window and computes topology via
`eeg_signal_topology.compute_topology_from_channels` -- not a hash of row_id
text. Unlike prior datasets, ds001787 windows have HIGHLY VARIABLE length (fixed
10s windows for Analysis A vs. probe_locked windows spanning 1s-165s+ for
Analysis B) -- `compute_topology_from_channels` operates on whatever sample
count it's given (more samples -> more stable correlation estimates, fewer ->
noisier but still computable), so no change to the underlying computation was
...
def load_level_m_window_features(m_windows_dir: str) -> list[dict]:
    ...
    if not p.exists():
        raise FileNotFoundError("Level M window features are required. Run run_ds001787_m_real first or use --mock-fixture.")
...
def build_level_t_omega_event(rows: list[LevelTRealTopologyRow]) -> dict:
    safe = "Local DS001787-style EEG windows were mapped into operational Level T topology telemetry candidates for future M+T residual testing."
    _validate_safe_text(safe)
    return {"dataset_id": "ds001787", "status": "operational_level_t", "n_rows": len(rows), "safe_claim": safe}
...
def write_level_t_topology_outputs(result: LevelTRealTopologyResult, out_dir: str) -> dict[str, str]:
    ...
    report = "\n".join([
        "# DS001787 Real/Local Level T Topology Extraction",
        "## Dataset/stage",
        "- dataset_id: ds001787",
