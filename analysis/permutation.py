...
def subject_blocked_permutation_test(
    ...
    value (mean/median across their windows), THEN permutes at the subject
    level. This is the direct, reusable encoding of the ds001787 report's
    ...
    subj = (
        ...
        .agg(**{
            value_col: (value_col, aggregate),
            group_col: (group_col, "first"),
        })
        .reset_index()
    )

    groups = subj[group_col].unique()
    if len(groups) != 2:
        raise ValueError(
            f"subject_blocked_permutation_test requires exactly 2 groups, got {list(groups)}"
        )
    g_a, g_b = sorted(groups, key=str)
    a = subj.loc[subj[group_col] == g_a, value_col].to_numpy(dtype=float)
    b = subj.loc[subj[group_col] == g_b, value_col].to_numpy(dtype=float)
